...
code_interviews_collection = mongodb.code_interviews_collection
...
async def start_code_session_handler(pdf: UploadFile, user_id: str):
    """
    Starts a new interview session:
    1. Extracts text from uploaded resume PDF.
    2. Generates an AI summary for interview context.
    ...
    # --- Step 4: Insert into DB ---
    result = await code_interviews_collection.insert_one(
        new_interview.model_dump(by_alias=True)
    )
    interview_id = result.inserted_id
